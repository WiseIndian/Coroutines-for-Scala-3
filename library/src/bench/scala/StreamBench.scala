...
import org.scalameter.japi.{JBench, ContextBuilder}
import scala.collection._
import coroutines.Macros._
import scala.language.implicitConversions


class StreamBench extends JBench.LocalTime /*OfflineReport*/ {

  //the problem is that the key is considered to be a KeyWithDefaultValue but we need a Key
  override def defaultConfig = 
    new ContextBuilder()
      .put( "exec.minWarmupRuns", 50 )
      .put( "exec.maxWarmupRuns", 100 )
      .put( "exec.benchRuns", 36 )
      .put( "exec.independentSamples", 4 )
      .put( "verbose", false)
      .build()
